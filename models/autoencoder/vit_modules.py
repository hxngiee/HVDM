...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, low, high, einops_from, einops_to, mask = None, rot_emb = None, **einops_dims):
        h = self.heads
...
class FeatureFusion(nn.Module):
    ...
    def forward(self, x, low, high, frame_mask = None):
        device = x.device
        f, hp, wp = x.size(2), x.size(3), x.size(4)
        n = hp * wp
        x = rearrange(x, 'b c f h w -> b (f h w) c')
        ...
        # time and space attention
        for (time_attn, spatial_attn, ff) in self.layers:
            x = time_attn(x, low, high, 'b (f n) d', '(b n) f d', n = n, mask = frame_mask, rot_emb = frame_pos_emb) + x
            x = spatial_attn(x, low, high, 'b (f n) d', '(b f) n d', f = f, rot_emb = image_pos_emb) + x
            x = ff(x) + x
